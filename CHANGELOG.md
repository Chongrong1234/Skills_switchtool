...
修复「v1.8.1 后 clone 仍卡住」:超大仓库 clone 的**服务端算 pack 静默期**超过空闲窗口。

- 现象:实测 `nexu-io/open-design`(400MB+ 浅克隆)时,git 打出「正克隆到…」后客户端
  **连续 2-3 分钟零输出**(GitHub 服务端为浅克隆计算 pack,期间不向客户端发任何进度),
  然后才以 ~0.5MB/s 流式下载十几分钟。v1.8.1 的 120s 空闲窗口把这段合法静默当成挂起,
  到点 SIGTERM——大仓库依旧装不上
- 改动(`src/core/library.ts`):默认空闲窗口 **120s → 10 分钟**(`SSW_GIT_TIMEOUT_MS` 仍可覆盖,
  测试调小);语义不变:有输出就续期,只有安静满窗口才杀。真挂起最迟 10 分钟报错
- 错误文案不变(「Ns 无任何输出,疑似网络挂起」,秒数随窗口动态显示)
